...
Original file is located at
    https://colab.research.google.com/drive/12VXU66uL_GByRgWZgxeNJz2jQGf7HoAW
...
class TextDataset(data.Dataset):
    ...
    def build_dictionary(self): 
        ...
        for i in range(len(self.examples)):
          sentence = self.examples[i][1]#.text
        ...
        for idx, w in enumerate(self.vocab):
          self.word2idx[w] = idx + 3
          self.idx2word[idx + 3] = w   
        ...
        for i in range(len(self.examples)):
          ...
          for word in sentence:
            ...
            if word not in self.word2idx:
              word = UNK
            senList.append(self.word2idx[word]) 
            #print('senlist: ', senList)
          senList.append(END) ###
          textual_ids.append(senList)
    ...
    def get_text(self, idx):
        ...
        ##### TODO #####
        rev = [0]*self.max_len
        #review = self.textual_ids[idx][1]
        review = self.examples[idx][1]

        rev_len = len(review)
        paddingLength = self.max_len - rev_len
      ...
      #  print('review', review)
       ...
       # print('len:', rev_len)  
        if paddingLength >0:
          for i in range(rev_len):
            if review[i] not in self.word2idx:
              review[i] = UNK
              rev[i] = 2
            else:
              rev[i] = self.word2idx[review[i].lower()]
          for i in range(rev_len, self.max_len):
            rev[i] = self.word2idx[PAD]
        if paddingLength <= 0:
       ...
       # print('rev ',rev)
        return torch.LongTensor(rev)

       # if paddingLength >0:
       #   x= review_str.zfill(paddingLength)
         # x = str(review_str).rjust(paddingLength, pad)
       #   review = int(x)    
       ...
       # print('x:', x)
        ...
        #return None
        #id_sen = self.textual_ids[idx][1]
        #new_id_sen = id_sen
        #while len(new_id_sen) < self.max_len:
           # new_id_sen.append(0)
        #return torch.LongTensor(new_id_sen)
    ...
    def get_label(self, idx):
        ...
        if (label.strip() == 'positive' or label.strip() == 'pos'):
          #return torch.LongTensor([1])
          return torch.squeeze(torch.LongTensor([1]))
        elif (label.strip() == 'negative' or  label.strip() == 'neg'):
...
class CNN(nn.Module):
    def __init__(self, vocab_size, embed_size, out_channels, filter_heights, stride, dropout, num_classes, pad_idx):
       ...
       # self.embedding = nn.Embedding(vocab_size, embed_size, padding_idx=pad_idx)
        ...
        #self.conv1 = nn.Conv2d(in_channels=1, out_channels=out_channels, kernel_size=tuple((filter_heights, embed_size)), stride = stride)#, padding = pad_idx)

        #self.conv2 = nn.Conv2d(in_channels=1, out_channels=out_channels, kernel_size=tuple((filter_heights, embed_size)), stride = stride)#, padding = pad_idx)
        #for filter in filter_heights:
        #    self.conv1 = nn.Conv2d(in_channels=1, out_channels=out_channels, kernel_size=(filter, embed_size), stride = stride)#, padding = pad_idx)

        #self.conv2 = nn.Conv2d(in_channels=1, out_channels=out_channels, kernel_size=([3,4,5], embed_size), stride = stride)#, padding = pad_idx)
...
#        self.fc = nn.Linear(out_channels * 2, 1)
    ...
    def forward(self, texts):
        ...
        x = torch.unsqueeze(texts, 1)
        #print('unsqueeze: ', texts)
        ...
        #
      #  for i in len(filter_heights):
      #    texts = self.conv1(texts)
      #    texts = texts.squeeze(1)
       # texts = self.conv2(texts)
       # texts = texts.squeeze(1)
     #   texts = nn.ReLU(texts)
    #    texts = torch.cat[texts[0],(out_channels*2)]
        
        x1 = torch.max(torch.squeeze(self.conv1(x), 3), 2)[0]
        x2 = torch.max(torch.squeeze(self.conv2(x), 3), 2)[0]
        x3 = torch.max(torch.squeeze(self.conv3(x), 3), 2)[0]
...
class RNN(nn.Module):
    def __init__(self, vocab_size, embed_size, hidden_size, num_layers, bidirectional, dropout, num_classes, pad_idx):
        ...
        #self.rnn = nn.LSTM(embed_size, hidden_size, num_layers=num_layers, bidirectional = bidirectional)
        ...
        if bidirectional: 
          direct = 2
          #print('dir: ', bidirectional)
        else: 
          direct = 1
        self.linear = nn.Linear(hidden_size*direct, num_classes)
    ...
    def forward(self, texts):
       ...
       # x = nn.utils.rnn.pack_padded_sequence(x, text_lens, batch_first=False, enforce_sorted=False)
        ...
        #out = torch.cat([texts, texts], dim=0)

       #### print('x: ', x.shape)

       # output = torch.cat((x, x), dim=0)
